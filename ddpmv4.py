...
def get_validation_data(args):
    ...
    # Here, we assume validation data is in a separate folder.
...
def validate(args, model, diffusion, device, val_loader):
    ...
    with torch.no_grad():
        for images, _ in val_loader:
            ...
            # For each batch, loss is averaged; multiply by batch size so that total_loss is 
            # the sum of errors over all images.
...
def compute_lpips(real_folder, fake_folder, device):
    ...
    # List and sort image file names from both folders
    ...
    for i in range(num_images):
        ...
        # Load images using LPIPS helper functions; these convert an image to a tensor in [-1,1]
...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Compute additional metrics (FID and LPIPS) comparing real images to the generated (fake) ones.
